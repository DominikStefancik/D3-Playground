...
const createTooltip = (chartData) => {
  ...
  d3.select(`.chartTooltip-${coinClass}`).remove();
  d3.select(`.overlay-${coinClass}`).remove();

  const tooltipGroup = parentGroup
    .append("g")
    // we have to add a different class for the tooltip in each chart
    // otherwise only the tooltip for the last chart will be shown
    ...
    .attr("class", `chartTooltip chartTooltip-${coinClass}`)
    .style("display", "none");
  ...
  parentGroup
    .append("rect")
    // we have to add a different class for the overlay in each chart
    // otherwise only the tooltip for the last chart will be shown
    .attr("class", `overlay overlay-${coinClass}`)
    .attr("width", width)
    .attr("height", height)
